...
//   );
// }
import { Send, BadgeCheck, Percent, BellDot, TrendingUp, CreditCard, } from "lucide-react";
import React from "react";
  ...
  const stats = [
    {
      title: "Sent",
      value: data.sent?.yesterday?.toLocaleString() || "0",
      previousValue: data.sent?.beforeYesterday?.toLocaleString() || "0",
      change: data.sent?.change,
      icon: Send,
    },
    {
      title: "Delivered",
      value: data.delivered?.yesterday?.toLocaleString() || "0",
      previousValue: data.delivered?.beforeYesterday?.toLocaleString() || "0",
      change: data.delivered?.change,
      icon: BadgeCheck,
    },
    {
      title: "Delivery Rate",
      value: `${data.deliveryRate?.yesterday || 0}%`,
      previousValue: `${data.deliveryRate?.beforeYesterday || 0}%`,
      change: data.deliveryRate?.change,
      icon: Percent,
    },
    {
      title: "Opens",
      value: data.opens?.yesterday?.toLocaleString() || "0",
      previousValue: data.opens?.beforeYesterday?.toLocaleString() || "0",
      change: data.opens?.change,
      icon: BellDot,
    },
    {
      title: "Open Rate",
      value: `${data.openRate?.yesterday || 0}%`,
      previousValue: `${data.openRate?.beforeYesterday || 0}%`,
      change: data.openRate?.change,
      icon: TrendingUp,
    },
    {
      title: "Recharge Conv",
      value: `${data.rechargeConv?.yesterday || 0}%`,
      previousValue: `${data.rechargeConv?.beforeYesterday || 0}%`,
      change: data.rechargeConv?.change,
      icon: CreditCard,
    },
  ];
      ...
      >
        {stats.map((item) => (
          <StatsCard
            ...
            positive={item.change?.direction === "up"}
          />
        ))}
      </div>
    </div>
